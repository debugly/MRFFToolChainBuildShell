...
if [ -z "$FF_ARCH" ]; then
    echo "You must specific an architecture 'x86_64, arm64'.\n"
    exit 1
fi
...
FFMPEG_CFG_FLAGS_ARM="$FFMPEG_CFG_FLAGS_ARM --disable-asm" # 暂时不开启 asm
...
if [ -f "./config.h" ]; then
    ...
    echo "config: $FFMPEG_CFG_FLAGS $FF_XCRUN_CC $FFMPEG_CFLAGS >> dep $FFMPEG_DEP_LIBS"
    ./configure \
        $FFMPEG_CFG_FLAGS \
        --cc="$FF_XCRUN_CC" \
        $FFMPEG_CFG_CPU \
